    ...
    if (journeyComplete) {
      ...
      function makeRose() {
        ...
        for (let p = 0; p < 6; p++) {
          ...
          petal.position.set(Math.cos(angle) * 0.08, 0.38, Math.sin(angle) * 0.08);
          petal.rotation.y = angle;
          rose.add(petal);
        }
        const bud = new THREE.Mesh(new THREE.SphereGeometry(0.07, 6, 6), centerMat);
        bud.position.y = 0.4;
        rose.add(bud);
        return rose;
      }
      ...
      rosePositions.forEach(([x, y, z], i) => {
        ...
        rose.scale.setScalar(0.75 + (i % 3) * 0.12);
        islandGroup.add(rose);
      });

      // Soft golden celebration light
      const bloomLight = new THREE.PointLight(0xfff5e6, 1.4, 12);
    ...
    <div className="fixed inset-0 bg-background text-on-surface overflow-hidden dark">
      ...
      >
        <div className="absolute inset-0 w-full h-full flex items-center justify-center pointer-events-none">
          <div className="relative w-[min(80vw,min(55vh,800px))] aspect-square max-w-full max-h-[min(70dvh,800px)]">
            <div
              ref={threeContainerRef}
              className={`absolute inset-0 w-full h-full pointer-events-auto transition-opacity duration-[3000ms] ${
